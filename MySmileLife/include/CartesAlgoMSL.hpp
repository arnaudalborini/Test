...
namespace MySmileLife{
    class CartesAlgoMSL final:public CardGame::CartesAlgo{
        ...
        public:
            CartesAlgoMSL(const CardGame::CarteGenerator* cc,const CardGame::GameMechanicsMonitor* mm):CardGame::CartesAlgo(cc,mm),jCarte(new JouerCarteMSL(cc,mm)){cout << "CartesAlgoMSL:"<<cc<<" "<<mm<<endl;}
            ~CartesAlgoMSL(){delete jCarte;}
            
            bool peutEtreJouee(const Player *pp, IdCarte id)const override{cout << "CartesAlgoMSL:peutEtreJouee"<<endl;return jCarte->peutEtreJouee(pp,id);}
